...
//Funcion to print the Nth node data in the BST 
int Nth_node(Node *temp,int n){
	if(temp==NULL){
		return 0;
	}
	if(count==n){
		printf("The Nth node data in inOrder is : %d ",temp->data);
		return 0;
	}
	Nth_node(temp->left,n);
	count++;
	Nth_node(temp->right,n);
}
